  ...
  return (
    <Container>
      ...
      <Card.Body>
        ...
        <div className='text-center'>
          { (connectionStatus?.requested===false && connectionStatus?.accepted===false) && <Button onClick={()=> handleConnectionsStatus('add')} variant="primary" className='me-2'>Добавить</Button> }
          { connectionStatus?.requested && <Button onClick={()=> handleConnectionsStatus('remove')} variant="primary" className='me-2'>Запрошено</Button> }
          { connectionStatus?.accepted && <Button onClick={()=> handleConnectionsStatus('remove')} variant="primary" className='me-2'>Удалить</Button> }
          { connectionStatus?.following && <Button onClick={()=> handleConnectionsStatus('unfollow')} variant="primary">Отписаться</Button> }
          { (connectionStatus?.following===false) && <Button onClick={()=> handleConnectionsStatus('follow')} variant="primary">Подписаться</Button> }
        </div>
        }
        {(userAuth.isAuthenticated && !connectionStatus && (userAuth.username!==userName)) &&
        <div className='text-center'>
          <Button onClick={()=> handleConnectionsStatus('add')} variant="primary" className='me-2'>Добавить</Button>
          <Button onClick={()=> handleConnectionsStatus('follow')} variant="primary">Подписаться</Button>
        </div>
        }
    ...
    <div className="container d-flex flex-wrap">
      <div className=" align-self-center align-items-center align-content-center">
        <h1 className="text-center pb-2">Последние мысли</h1>
      </div>
    {blogsdata.map((blog) => 
      <>
